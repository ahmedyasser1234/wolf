...
export default function ProductsTab({ collectionId, onProductClick, onPreview, showConfirm }: ProductsTabProps) {
    ...
    const submitMutation = useMutation({
        mutationFn: async () => {
            ...
            formData.append("sizes", "[]"); // Kept empty for backwards compatibility 
                                    ...
                                    <div className="space-y-6 bg-gradient-to-br from-blue-900/20 to-cyan-900/20 p-4 sm:p-12 rounded-[1.5rem] md:rounded-[40px] border border-gray-800">
                                        <div className="flex items-center justify-between">
                                            ...
                                            <Button
                                                variant="outline"
                                                size="sm"
                                                onClick={() => setColorVariants([...colorVariants, { colorName: "", colorCode: "#000000", quantity: 0, imageFiles: [], existingImages: [] }])}
